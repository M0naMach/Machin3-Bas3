...
Principle: "Never guess--ask, flag, or escalate." This is enforced at the prompt level, not via external tooling.`,
    refinement: `The prompt architecture follows a single ownership rule: one agent, one file. Each agent file exports a system prompt, a set of behavioral constraints, and any domain-specific context the agent needs. They are imported at the API route level.
...
**Every agent has a lane. Every lane has a volume. Every volume has a home.**`,
    result: `The answer here is the same as it is in any well-maintained codebase: single responsibility, version history, and a review process that leaves a record. The Ecosystem project architecture extends that same thinking to every layer of the project, from configuration to documentation to AI behavior.
